public class PeekIndexInMountain
{
    static int searchPeek(int[] arr){
       int n=arr.length;
        int start=1,end=n-2; //0 and n-1 element cannot be the peak bt it can be the mid so thats the problem but it cannot the peak so we dont need to check for them 
        while(start<=end){
